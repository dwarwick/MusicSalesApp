...
public partial class AlbumPlayerModel : BlazorBase, IAsyncDisposable
{
    ...
    protected string GetTrackStreamUrl(int index)
    {
        if (_albumInfo == null || index >= _albumInfo.Tracks.Count) return "";
        return $"api/music/{SafeEncodePath(_albumInfo.Tracks[index].Name)}";
    }
    ...
    protected string GetTrackAlbumArtUrl(int index)
    {
        ...
        // For album tracks, we need to find individual track album art if it exists
        var track = _albumInfo.Tracks[index];
        var baseName = Path.GetFileNameWithoutExtension(Path.GetFileName(track.Name)).ToLowerInvariant();
        var folder = (Path.GetDirectoryName(track.Name)?.Replace("\\", "/") ?? "").ToLowerInvariant();
        
        // Try to get individual track art, fallback to album cover
